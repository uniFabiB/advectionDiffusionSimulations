...
scale = 1
freq = 1
ic_u = project(as_vector([scale*sin(freq*2*pi*x/L_x), scale*sin(freq*2*pi*y/L_y)]), W.sub(0))
ic_v = 0
u_.assign(ic_u)
v_.assign(ic_v)

print("inital values assigned ",datetime.datetime.now)
...
F = (inner((u - u_)/timestep, u_test)
    ...
    + inner(v, u_test)
    - inner(grad(v), grad(u_test))
    + inner(v, v_test)
    + inner(grad(u), grad(v_test))
    )*dx
#################################
