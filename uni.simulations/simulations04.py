...
u_adv = project(as_vector([0, sin(freq_x*2*pi*x/L_x)]), V)

#u_adv = project(as_vector([sin(freq_x*2*pi*y/L_y), 0]), V)

#u_adv = project(as_vector([1, 1]), V)
#u_adv.assign(0)
    
u_0Random1 = 2*(2*np.random.rand(n_x*n_y)-1)       # zwischen 0 und 1 -> -1 und 1
u_0Random2 = 2*(2*np.random.rand(n_x*n_y)-1)       # zwischen 0 und 1 -> -1 und 1
...
L2timeValues = np.zeros((3, numberOfTimesteps+2))
L2timeValues[:,0] = [L2norm[0],L2norm[1],L2norm[2]]
#ic_theta = Function(V,[u_0Random1,u_0Random2])
outfile_timeFunctions = File(output_dir_path + "/../data/temp/timeFunctions.pvd")
L2normXTimeFunction = Function(VecSpaceTime,L2timeValues[0,:])
L2normYTimeFunction = Function(VecSpaceTime,L2timeValues[1,:])
L2normTotTimeFunction = Function(VecSpaceTime,L2timeValues[2,:])
outfile_timeFunctions.write(project(L2normXTimeFunction, VecSpaceTime, name="L^2x(t)"),project(L2normYTimeFunction, VecSpaceTime, name="L^2y(t)"),project(L2normTotTimeFunction, VecSpaceTime, name="L^2tot(t)"), time=0)
...
while (t < T_end):
    ...
    L2timeValues[:,t_i] = getL2Norms(theta.dat.data)
    L2normXTimeFunction = Function(VecSpaceTime,L2timeValues[0,:])
    L2normYTimeFunction = Function(VecSpaceTime,L2timeValues[1,:])
    L2normTotTimeFunction = Function(VecSpaceTime,L2timeValues[2,:])
    outfile_timeFunctions.write(project(L2normXTimeFunction, VecSpaceTime, name="L^2x(t)"),project(L2normYTimeFunction, VecSpaceTime, name="L^2y(t)"),project(L2normTotTimeFunction, VecSpaceTime, name="L^2tot(t)"), time=t)
